...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("my solution", |b| {
        b.iter(|| {
            let s = black_box(String::from("bbcbabaeegeffgmumr"));
            Solution::count_palindromic_subsequence(s);
        })
    });

    c.bench_function("outer solution", |b| {
        b.iter(|| {
            let s = black_box(String::from("bbcbabaeegeffgmumr"));
            Solution::count_palindromic_subsequence2(s);
        })
    });
}
